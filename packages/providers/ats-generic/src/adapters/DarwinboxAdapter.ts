...
export class DarwinboxAdapter implements ApplicationAdapter {
  ...
  async fill(page: unknown, profile: unknown, resumePath: string): Promise<ApplicationOutcome> {
    ...
    await frame.fill('input[name*="email"], input[id*="email"]', email, { timeout: 5000 }).catch(() => {});
  ...
  private async handleCTCAndNoticePeriod(frame: Page | Frame, candidate: CandidateProfileForApplication) {
    if (candidate.expectedCtc) {
      await frame.fill('input[name*="ctc" i], input[id*="ctc" i]', String(candidate.expectedCtc), { timeout: 1000 }).catch(() => {});
    }
    if (candidate.noticePeriod) {
      await frame.fill('input[name*="notice" i], input[id*="notice" i]', String(candidate.noticePeriod), { timeout: 1000 }).catch(() => {});
    }
  ...
  async submit(page: unknown, submitLocator?: string): Promise<SubmissionResult> {
    ...
    await frame.click(submitLocator, { timeout: 5000 }).catch(() => {
    ...
    try {
      ...
      const confirmationIdMatch = url.match(/id=([a-zA-Z0-9_-]+)/);
