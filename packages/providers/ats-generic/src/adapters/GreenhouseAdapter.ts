...
export class GreenhouseAdapter implements ApplicationAdapter {
  ...
  async fill(page: unknown, profile: unknown, resumePath: string): Promise<ApplicationOutcome> {
    ...
    await browserPage.waitForSelector('input[name="first_name"]', { state: 'visible', timeout: 5000 }).catch(() => {
      throw new Error('MISSING_SELECTOR:input[name="first_name"]');
    });
    ...
    await browserPage.fill('input[name="email"]', email, { timeout: 5000 });
  ...
  async submit(page: unknown, submitLocator?: string): Promise<SubmissionResult> {
    ...
    await browserPage.click(submitLocator, { timeout: 5000 }).catch(() => {
      throw new Error('MISSING_SELECTOR:submit_button');
    });

    try {
      const confirmation = await browserPage.waitForSelector('h1:has-text("Thank you"), .application-success', {
        timeout: 10000,
      });
