...
export class AshbyAdapter implements ApplicationAdapter {
  ...
  async fill(page: unknown, profile: unknown, resumePath: string): Promise<ApplicationOutcome> {
    ...
    if (emailInput) {
      await emailInput.fill(candidate.user?.email || '');
    }
    ...
    for (const select of selects) {
      ...
      if (labelText.includes('gender') || labelText.includes('race') || labelText.includes('veteran') || labelText.includes('disability') || labelText.includes('eeo')) {
