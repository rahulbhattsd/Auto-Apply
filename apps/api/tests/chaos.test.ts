...
test('Chaos Testing - Worker Crash Recovery & Dead Letter Recording', async (t) => {
  const queue = new Queue(QUEUE_NAMES.AGENT_TASKS, { connection });
    ...
    if (failedJob && failedJob.id === job.id) {
      ...
      await prisma.deadLetter.create({
        data: {
          ...
          error: err.message,
          attemptCount: failedJob.attemptsMade || 1,
